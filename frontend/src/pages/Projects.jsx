...
function Projects() {
  const { user } = useAuth(); // NEW: Tap into JWT state
  ...
  }, [page, searchTerm, selectedDomain, user]); // Added user to dependencies

  return (
    <div className="min-h-screen bg-offWhite font-sans text-gray-800 relative">
      <TopNavbar subtitle="PROJECT DIRECTORY" />
      
      {/* HTML Layout is identical from your original file */}
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
          ...
          <div className="flex gap-4 w-full md:w-auto">
            <select className="border border-gray-300 rounded-lg p-2.5 bg-white text-sm focus:ring-brickRed focus:border-brickRed" value={selectedDomain} onChange={(e) => setSelectedDomain(e.target.value)}>
            ...
            <div className="relative w-full md:w-64">
              ...
              <input type="text" className="block w-full p-2.5 pl-10 text-sm border border-gray-300 rounded-lg bg-white" placeholder="Search projects..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
          </div>
        </div>
          ...
          <>
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-6 mt-12">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className={`px-5 py-2.5 rounded-lg font-bold transition-colors ${page === 1 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-brickRed border border-brickRed hover:bg-red-50 shadow-sm'}`}>&larr; Previous</button>
                <span className="text-gray-600 font-medium">Page <span className="font-bold text-gray-900">{page}</span> of {totalPages}</span>
                <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className={`px-5 py-2.5 rounded-lg font-bold transition-colors ${page === totalPages ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-brickRed border border-brickRed hover:bg-red-50 shadow-sm'}`}>Next &rarr;</button>
              </div>
            )}
          </>
        )}
    ...
    <div className="bg-white rounded-xl shadow-sm hover:shadow-md border border-gray-100 p-6 flex flex-col justify-between h-64 transition-transform duration-200 hover:-translate-y-1">
        ...
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="flex flex-wrap gap-1 justify-end max-w-[60%]">
            {project.domain && project.domain.slice(0, 2).map((d, index) => (
              <span key={index} className={`px-2 py-1 rounded text-xs font-medium border ${getDomainColor(d)}`}>{d}</span>
            ))}
          </div>
        </div>
      ...
      <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
        <div className="flex flex-col">
          ...
          <span className="text-sm font-semibold text-gray-700">{project.professorName || "Unknown Faculty"}</span>
        </div>
